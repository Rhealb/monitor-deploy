...
# To start admin-center
/bin/admin-center -config-path=/opt/mntcephutils/engine/conf/alerts \
                  -mysql-server=${MYSQLSERVER} \
                  -console-grpc=${CONSOLEGRPC} \
                  -prometheus=${PROMETHEUS} \
                  -prometheus-amah=${PROMETHEUSAMAH} \
                  -osticket-switch=${OSTICKETSWITCH} \
                  -osticket-server=${OSTICKETSERVER} \
                  -s3bucket=${S3BUCKET} \
                  -s3key=${S3KEY} \
                  -timeout=${TIMEOUT}