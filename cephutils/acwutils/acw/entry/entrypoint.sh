...
# To start admin-center-web
/bin/admin-center-web -cephpath="/opt/mntcephutils/acw/scripts" \
                      -script-exporter=${SCRIPT_EXPORTER} \
                      -automation=${AUTOMATION} \
                      -storage-type=${STORAGE_TYPE} \
                      -alertmanager=${ALERT_MANAGER} \
                      -utron-config-path="/opt/mntcephutils/acw/conf" \
                      -s3bucket=${S3BUCKET} \
                      -s3key=${S3KEY} \
                      -timeout=${TIMEOUT}